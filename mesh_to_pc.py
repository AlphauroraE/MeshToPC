...
#Note: Change inputpath depending on the computer
...
#Note: Change parent_dir depending on the computer
...
for entry in obj: # goes through the input folder
    ...
    if inputfile != ".DS_Store": # The first entry is always .DS_Store, and this is not a mesh file, so ignore it
        if inputfile.find("_") != -1: # mesh files without underscore were returning -1 and creating a name with cut off letters
        ...
        #Note: Alter function call as needed
        mesh_to_pc(inputpath,inputfile,5000,outputfile,outputpath) # call the function defined above. Alter as needed!
